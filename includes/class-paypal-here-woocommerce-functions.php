...
if ( ! function_exists( 'print_attribute_radio' ) ) {
		function print_attribute_radio( $checked_value, $value, $label, $name ) {
			...
			printf( '<div><label class="btn btn-outline-secondary" for="%3$s"><input type="radio" class="paypal_here_variation_radio" name="%1$s" value="%2$s" id="%3$s" %4$s>%5$s</label></div>', $input_name, $esc_value, $id, $checked, $filtered_label );
		}
	}