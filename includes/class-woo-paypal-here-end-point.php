...
class Woo_PayPal_Here_End_Point {
    ...
    public function __construct() {
        ...
        add_action('init', array($this, 'angelleye_woo_paypal_here_add_endpoints'), 0);
        add_filter( 'status_header', array($this, 'angelleye_woo_paypal_here_status_header'), 0, 2);
        ...
        add_action('wp_ajax_angelleye_woo_paypal_here_update_api_key', array($this, 'angelleye_woo_paypal_here_update_api_key'));
        ...
        add_action('angelleye_woo_paypal_here_view_products_body_content', array($this, 'angelleye_woo_paypal_here_view_products_body_content'), 5);
        add_action('angelleye_woo_paypal_here_view_pending_orders_body_content', array($this, 'angelleye_woo_paypal_here_view_pending_orders_body_content'), 5);
        add_action('angelleye_woo_paypal_here_view_order_billing_body_content', array($this, 'angelleye_woo_paypal_here_view_order_billing_body_content'), 5);
        add_action('angelleye_woo_paypal_here_view_order_shipping_body_content', array($this, 'angelleye_woo_paypal_here_view_order_shipping_body_content'), 5);
        add_action('template_redirect', array($this, 'angelleye_woo_paypal_here_handle_submit_action'), 20);
        add_action('angelleye_woo_paypal_here_view_pending_orders_details_body_content', array($this, 'angelleye_woo_paypal_here_view_pending_orders_details_body_content'), 10);
        ...
        if (!is_admin()) {
            add_filter('query_vars', array($this, 'angelleye_woo_paypal_here_add_query_vars'), 0);
    ...
    public function angelleye_woo_paypal_here_parse_request() {
        global $wp;
        $this->paypal_here_settings = get_option('woocommerce_angelleye_woo_paypal_here_settings');
    ...
    public function angelleye_woo_paypal_here_page_endpoint_wp_title($title) {
        global $wp_query, $wp;
        $wp->query_vars;
        if (!is_null($wp_query) && !is_admin() && is_main_query() && $this->is_angelleye_woo_paypal_here_endpoint_url($title)) {
            ...
            remove_filter('the_title', 'angelleye_woo_paypal_here_page_endpoint_wp_title');
        }
        return $title;
    }

    public function angelleye_woo_paypal_here_page_endpoint_title($title) {
        global $wp_query, $wp;
        $wp->query_vars;
        if (!is_null($wp_query) && !is_admin() && is_main_query() && $this->is_angelleye_woo_paypal_here_endpoint_url($title)) {
            add_filter('show_admin_bar', '__return_false');
            ...
            remove_filter('the_title', 'angelleye_woo_paypal_here_page_endpoint_title');
        }
        return $title;
    }
    ...
    public function angelleye_woo_paypal_here_revoke_key() {
        global $wpdb;
        ...
        $this->angelleye_woo_paypal_here_remove_key($this->paypal_here_settings['key_id']);
        ...
        update_option('woocommerce_angelleye_woo_paypal_here_settings', $this->paypal_here_settings);
        $data['message'] = 'API key revoked successfully.';
        wp_send_json_success($data);
    }

    public function angelleye_woo_paypal_here_update_api_key() {
        if (empty($this->paypal_here_settings)) {
            $this->paypal_here_settings = get_option('woocommerce_angelleye_woo_paypal_here_settings');
        }
        ob_start();
    ...
    public function angelleye_woo_paypal_here_remove_key($key_id) {
        try {
            global $wpdb;
    ...
    public function angelleye_woo_paypal_here_dashboard_body_content() {
    ...
    public function angelleye_woo_paypal_here_view_products_body_content() {
        try {
            $this->angelleye_woo_paypal_here_display_product_list();
        } catch (HttpClientException $ex) {
    ...
    public function angelleye_woo_paypal_here_display_product_list() {
        $this->result = $this->here_rest_api->angelleye_woo_paypal_here_get_product();
        $this->angelleye_woo_paypal_here_get_product_list();
        include $this->plugin_path() . '/templates/' . 'products_search.php';
        include $this->plugin_path() . '/templates/' . 'products.php';
    }

    public function angelleye_woo_paypal_here_get_product_list() {
    ...
    public function angelleye_woo_paypal_here_view_pending_orders_body_content() {
        try {
            $this->angelleye_woo_paypal_here_display_order_list();
        } catch (HttpClientException $ex) {
            echo print_r($ex->getMessage(), true);
        } catch (Exception $ex) {
            echo print_r($ex->getMessage(), true);
    ...
    public function angelleye_woo_paypal_here_display_order_list() {
        $this->result = $this->here_rest_api->angelleye_woo_paypal_here_get_pending_order();

        $this->angelleye_woo_paypal_here_get_order_list();
        include $this->plugin_path() . '/templates/' . 'orders_search.php';
        include $this->plugin_path() . '/templates/' . 'orders.php';
    }

    public function angelleye_woo_paypal_here_get_order_list() {
        if (!empty($this->result)) {
            foreach ($this->result as $key => $value) {
    ...
    public function angelleye_woo_paypal_here_view_order_billing_body_content() {
        include $this->plugin_path() . '/templates/' . 'order_billing.php';
    }

    public function angelleye_woo_paypal_here_view_order_shipping_body_content() {
        include $this->plugin_path() . '/templates/' . 'order_shipping.php';
    }

    public function angelleye_woo_paypal_here_before_body_content() {
    ...
    public function angelleye_woo_paypal_here_view_pending_orders_details_body_content() {
    ...
    public function paypal_here_get_copon_code() {
        $items = array();
        $this->result = $this->here_rest_api->angelleye_woo_paypal_here_get_coupons();
        if (!empty($this->result)) {
            foreach ($this->result as $key => $value) {
                $items[] = $value['code'];
            }
    ...
    public function angelleye_woo_paypal_here_status_header($status_header, $code) {
        global $wp, $wp_query;
        if(!empty($wp->query_vars['name']) && !empty($this->paypal_here_settings['paypal_here_endpoint_url'])) {
            if($wp->query_vars['name'] == $this->paypal_here_settings['paypal_here_endpoint_url']) {
